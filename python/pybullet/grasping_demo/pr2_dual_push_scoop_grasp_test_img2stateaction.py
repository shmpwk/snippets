        ...
        try:
            ...
            while (try_count != try_num):
                ...
                for i in range(data_length):
                    ...
                    # Inference
                    encoded = self.ae(img).to(device).reshape(1,1,8+6) #batch, time length, vector 
                    output = self.model(encoded.to(device)).to(device)
                    rx, ry, lx, ly, rtheta, ltheta, rw, lw, _, _, _, _, _, _ = output[0,0,:].cpu().detach().numpy()
